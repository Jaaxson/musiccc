...
async def join_group(client, message):
    ...
    try:
        ...
        print(e)
        await message.reply_text(
            f"✅ ** عفوا الحساب المساعد بلفعل في المجموعه**",
        )
    except Exception as e:
        print(e)
        await message.reply_text(        
...
async def leave_one(client, message):
    try:
        ...
        await message.reply_text(
            "❌ **لم يتمكن الحساب المساعد من المغادره ربما يكون بسبب الضغط.**\n\n**» انتظر او قم بطردي يدويا**"
        )

        return


@Client.on_message(command(["leaveall","ادر الجميع", f"leaveall@{BOT_USERNAME}"]))
@sudo_users_only
